...
#[export] Instance listF : Functor list := { fmap := @list_fmap }.
#[export] Instance listA : Applicative list := { pure := fun _ x => x :: nil
                                     ; liftA := @list_liftA }.
#[export] Instance listM : Monad list := 
  { bind := @list_bind }.

#[export] Instance listF_correct : Functor_Correct list.
Proof.
  constructor.
...
#[export] Instance listA_correct : Applicative_Correct list.
Proof.
  constructor.
...
#[export] Instance listM_correct : Monad_Correct list.
Abort.
...
#[export] Instance optionF : Functor option := { fmap := @option_fmap}.
#[export] Instance optionA : Applicative option := { pure := @Some;
                                           liftA := @option_liftA}.
#[export] Instance optionM : Monad option :=
  { bind := fun  A m B f => match m with None => None | Some a => f a end
  }.
#[export] Instance optionM_Laws : Monad_Correct option.
Proof. split.
  - destruct a; auto.
  - intros; auto.
...
Defined.
#[export] Instance optionT_T : MonadTrans optionT := {liftT := @optionT_liftT}.
...
#[export] Instance optionT_F {f} `{Functor f} : Functor (optionT f) := 
    {fmap := @optionT_fmap f _}.
#[export] Instance optionT_A {f} `{Applicative f} : Applicative (optionT f) :=
  { pure := @optionT_pure f _ _;
    liftA := @optionT_liftA f _ _ }.
#[export] Instance optionT_M {m} `{Monad m} : Monad (optionT m) :=
...
#[export] Instance readerF E : Functor (Reader E) :=
 { fmap := @reader_fmap E }.
#[export] Instance readerA E : Applicative (Reader E) :=
 { pure := fun  A (a:A) e=> a;
   liftA := @reader_liftA E }.
#[export] Instance readerM (E : Type): Monad (Reader E) :=
 { bind := @reader_bind E }.
(*
(* Checking the 3 laws *)
 - (* unit_left *)
...
#[export] Instance stateF {A} : Functor (State A) :=
    { fmap := @state_fmap A }.
#[export] Instance stateA {A} : Applicative (State A) :=
    { pure := fun  A a s=> (a,s);
      liftA := @state_liftA A }.
#[export] Instance stateM {A} : Monad (State A) :=
    { bind := @state_bind A }.


#[export] Instance stateF_correct {A} : Functor_Correct (State A).
  Proof.
    split; intros;
...
#[export] Instance stateA_correct {A} : Applicative_Correct (State A).
  Proof. 
    split; intros;
      apply functional_extensionality; intros op; 
...
#[export] Instance stateM_correct {A} : Monad_Correct (State A).
  Proof.
