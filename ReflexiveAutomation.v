...
Require Export HeisenbergFoundations.Automation.
...
Proof.
  ...
  intros [Hfst%QCeqb_eq Hsnd%listPOrd.eqb_eq_true].
  unfold translateQ, QTType_to_TType.
  unfold translate.
  cbn.
...
Ltac split_pred_eq :=
  lazymatch goal with 
  | |- pred_eq (Cap _) (Cap _) => eapply pred_eq_Cap; 
...
Inductive Capeq : forall {n}, relation (list (AType n)) :=
  | Capeq_nil {n} : @Capeq n [] []
  | Capeq_skip {n} (a : AType n) (la la' : list (AType n)) : 
      Capeq la la' -> Capeq (a :: la) (a :: la')
  | Capeq_swap {n} (a a' : AType n) (la : list (AType n)) : 
      Capeq (a :: a' :: la) (a' :: a :: la)
  | Capeq_Aeq {n} (a a' : AType n) (la la' : list (AType n)) :
    a ≈ a' -> Capeq la la' -> Capeq (a :: la) (a' :: la')
  | Capeq_sym {n} (la la' : list (AType n)) : Capeq la la' -> Capeq la' la
  | Capeq_trans {n} (la la' la'' : list (AType n)) : Capeq la la' -> Capeq la' la'' -> Capeq la la''.

Lemma Perm_Capeq {n} (la la' : list (AType n)) :
  Permutation la la' -> Capeq la la'.
Proof. intros H.
  induction H; constructor; auto.
  apply Capeq_sym.
  apply Capeq_trans with (la' := l'); auto.
Qed.

Lemma pred_eq_Cap' {n} (l l' l'' : list (AType n)) : 
  Capeq l' l'' -> Forall2 atype_eq l l' -> pred_eq (Cap l) (Cap l'').
Proof. intros Ceq Hll'.
  pose (pred_eq_Cap l l' Hll') as H.
  unfold pred_eq in *.
  setoid_rewrite H.
  clear H Hll' l.
  induction Ceq; intros; simpl in *; auto.
  - split; auto.
  - rewrite ! Forall_cons_iff.
    split; intros [H [H0 H1]];
    repeat (split; auto).
    + assert (WF_Matrix v /\ Forall (fun a0 : AType n => vecSatisfies v (translateA a0)) la) by auto.
      rewrite IHCeq in H2.
      destruct H2. auto.
    + assert (WF_Matrix v /\ Forall (fun a0 : AType n => vecSatisfies v (translateA a0)) la') by auto.
      rewrite <- IHCeq in H2.
      destruct H2. auto.
  - rewrite ! Forall_cons_iff.
    split; intros [H [H0 [H1 H2]]];
      repeat (split; auto).
  - rewrite ! Forall_cons_iff.
    apply Aeq_atype_eq_subrelation in H.
    unfold atype_eq in H.
    rewrite H.
    split; intros [H0 [H1 H2]];
      repeat (split; auto).
    + assert (WF_Matrix v /\ Forall (fun a0 : AType n => vecSatisfies v (translateA a0)) la) by auto.
      rewrite IHCeq in H3. destruct H3. auto.
    + assert (WF_Matrix v /\ Forall (fun a0 : AType n => vecSatisfies v (translateA a0)) la') by auto.
      rewrite <- IHCeq in H3. destruct H3. auto.
  - symmetry. auto.
  - rewrite IHCeq1. auto.
...
Ltac validate_refl_sorted :=
  apply prep_validate_refl_lemma'';
    ...
    try (apply Capeq_sym;
         eapply Capeq_trans;
         [repeat (eapply Capeq_Aeq; 
                  [eapply Aeq_trans;
                   [apply (AType_Aeq_simplify_lemma _ _ _)
                   | apply Permutation_Aeq; apply TOrd.Permuted_sort]
                  | try apply Capeq_nil])
         | idtac]);
    try (apply Aeq_atype_eq_subrelation;
         apply Permutation_Aeq;
         apply TOrd.Permuted_sort);
    try (apply Perm_Capeq;
         eapply Permutation_trans;
         [apply AOrd.Permuted_sort 
...
Definition normalizeA (m : nat) {n : nat} (La : list (AType n)) :=
  (map TtoA (normalize m (map AtoT La))).
...
Ltac solvePlaceholder_refl_internal_normalized m :=
  eapply (normalization_admissible' m);
  [ solvePlaceholder_refl_internal
  | f_equal 
  | repeat constructor
  | unfold AtoT; simpl; reflexivity
  | repeat (constructor; try lia)
  | solveNormalize; reflexivity].

Ltac solvePlaceholder_refl_normalized m :=
  let G := fresh "G" in
  eexists;
   match goal with
   | |- ?g => assert (G : g); [ solvePlaceholder_refl_internal_normalized m |  ]
   end.


Ltac validateSep := 
  eapply CONS;
  [ solveNormalize; Csimpl; apply ID_implies
  | solveNormalize; Csimpl; validateCapImpliesSep
  | unfold compose, to_fun, collect_fun; simpl; Csimpl;
    eapply CONS; 
    [ apply ID_implies | try apply CapIntro 
    | solvePlaceholder_refl_internal ] ];
  validateCapImpliesCap.

Ltac validateSep_normalized m := 
  eapply CONS;
  [ solveNormalize; Csimpl; apply ID_implies
  | solveNormalize; Csimpl; validateCapImpliesSep
  | unfold compose, to_fun, collect_fun; simpl; Csimpl;
    eapply CONS; 
    [ apply ID_implies | try apply CapIntro
    | solvePlaceholder_refl_internal_normalized m ] ];
  validateCapImpliesCap.
...
Ltac validate_refl' :=
  solveNormalize;
  match goal with
  | |- {{ _ }} _ {{ Sep _ }} => validateSep
  | |- {{ Cap _ }} _ {{ _ }} => validate_refl_sorted
  | |- _ => validate_refl
  end.

Ltac validate_refl'_normalized m :=
  solveNormalize;
  match goal with
  | |- {{ _ }} _ {{ Sep _ }} => validateSep_normalized m
  | |- {{ Cap _ }} _ {{ _ }} => solvePlaceholder_refl_internal_normalized m
  | |- _ => validate_refl
  end.
